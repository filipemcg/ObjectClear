...
from schemas.sqs import JobEnvelope
import json
...
def process_image(task_definition: JobEnvelope) -> JobStatus:
    try:
        ...
        phone: str = task_definition.payload.meta.phone
        domain: str = task_definition.payload.meta.domain
        identifier: str = Utils.generate_identifier()

        image_helper: ImageHelper = ImageHelper.from_url(task_definition.payload.meta.url)
        image_helper.resize()
        ...
        dynamo_item = JobStatus(
            hash="OBJECT_CLEAR",
            range=task_definition.request_id,
            meta=MetaStatus(
                status="COMPLETED",
                data=MetaDataV1(
                    version="v1",
                    original_uri=original_image_uri,
                    removed_uri=watermark_removed_uri,
                ),
            ),
        )
        ...
        for message in messages:
            try:
                ...
                if task_definition.payload.job == 'OBJECT_REMOVAL':
                    dynamo.put_item(JobStatus(hash="OBJECT_CLEAR", range=task_definition.request_id, meta=MetaStatus(status='PENDING', data=MetaDataV1(version="v1"))))
                    result: JobStatus = process_image(task_definition)
                    dynamo.put_item(JobStatus(hash="OBJECT_CLEAR", range=task_definition.request_id, meta=result.meta))
            except Exception as e:
